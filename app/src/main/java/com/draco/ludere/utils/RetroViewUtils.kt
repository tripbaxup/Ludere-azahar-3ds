...
class RetroViewUtils(private val activity: Activity) {
    ...
    private val fastForwardSpeed =
        activity.resources.getInteger(
            R.integer.config_fast_forward_multiplier
        )


    fun restoreEmulatorState(retroView: RetroView) {

        // Always boot emulator at normal speed
        // Prevents saved fast-forward state
    ...
    fun preserveEmulatorState(retroView: RetroView) {
        ...
        with(sharedPreferences.edit()) {
            ...
            // Always save normal speed
            putInt(
                activity.getString(R.string.pref_frame_speed),
                1
            )
    ...
    fun loadState(retroView: RetroView) {
        ...
        retroView.view.unserializeState(
            stateBytes
        )
    }
    ...
    fun loadTempState(retroView: RetroView) {
        ...
        val stateBytes =
            storage.tempState.inputStream().use {
    ...
    fun fastForward(retroView: RetroView) {
        ...
        retroView.view.frameSpeed =
            if (retroView.view.frameSpeed == 1) {
